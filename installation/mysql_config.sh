...
get_mysql_credentials() {
    ...
    if test_mysql_connection "$mysql_host" "$mysql_user" "$mysql_password"; then
        # Save credentials to shell config file
...
EOF
        ...
        # Save credentials to MySQL config file for passwordless access
        cat > installation/mysql_config.cnf << EOF
[client]
host=$mysql_host
user=$mysql_user
...
EOF
        ...
        # Set proper permissions on MySQL config file
        chmod 600 installation/mysql_config.cnf
        
        print_success "MySQL credentials saved to installation/mysql_credentials.conf"
        print_success "MySQL config file created at installation/mysql_config.cnf"
        print_warning "Keep these files secure and do not share them!"
